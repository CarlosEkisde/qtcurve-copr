...
%if 0%{?qt6}
BuildRequires:  pkgconfig(Qt6Gui) pkgconfig(Qt6Widgets) pkgconfig(Qt6Svg) pkgconfig(Qt6DBus)
...
Requires: %{name}-kf6%{?_isa} = %{version}-%{release}
...
%package qt6
Summary: QtCurve Qt6 style support
Requires: %{name}-libs%{?_isa} = %{version}-%{release}
...
%package kf6
Summary: QtCurve KF6/Plasma support
Requires: %{name}-qt6%{?_isa} = %{version}-%{release}
## not sure yet -- rex
#Requires: plasma-workspace
%description kf6
...
%cmake \
  ...
  -DDENABLE_QT6:BOOL=%{?qt6:ON}%{!?qt6:OFF} \
  -DQTC_QT6_ENABLE_KDE:BOOL==%{?qt6:ON}%{!?qt6:OFF}
...
rm -rfv %{buildroot}/usr/share/locale
rm -rfv %{buildroot}/usr/lib/debug
...
%files qt6
%{_qt6_plugindir}/styles/qtcurve.so
...
%files kf6
%{_kf6_qtplugindir}/kstyle_qtcurve6_config.so
%{_kf6_datadir}/kstyle/themes/qtcurve.themerc
#%{_kf6_datadir}/kxmlgui6/QtCurve/


%changelog
* Sat Nov 29 2025 Infinality <infinality@infinality.net> - 1.9.1-33
- Experimental QT6 build including configuration dialog.
